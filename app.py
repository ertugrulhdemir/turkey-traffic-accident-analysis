...
c4.metric("Most Dangerous Province (2025)", df_2025.loc[df_2025['deaths_per_1000'].idxmax(), 'province'])
st.divider()
...
with tab5:
    ...
    # Forecast table
    st.divider()
    st.markdown("### 📋 2026–2030 Forecast Summary")
    st.dataframe(
        forecast_df[['year', 'pred_accidents', 'pred_deaths', 'registered_vehicles']]
        .rename(columns={'year': 'Year', 'pred_accidents': 'Predicted Accidents',
                         'pred_deaths': 'Predicted Deaths',
                         'registered_vehicles': 'Est. Registered Vehicles'})
        .assign(**{'Predicted Accidents': lambda x: x['Predicted Accidents'].map('{:,.0f}'.format),
                   'Predicted Deaths': lambda x: x['Predicted Deaths'].map('{:,.0f}'.format),
                   'Est. Registered Vehicles': lambda x: x['Est. Registered Vehicles'].map('{:,.0f}'.format)}),
        use_container_width=True, hide_index=True
    )
    ...
    if has_full_features:
        ...
        'Feature': ['Year', 'Registered Vehicles', 'Driver Fault Ratio', 'Vehicles Involved'],
        'Importance': fi_acc
        }).sort_values('Importance', ascending=False)
        fig_imp = px.bar(importance_df, x='Importance', y='Feature', orientation='h',
                         title='Which feature drives accident prediction more?',
                         color='Feature',
                         color_discrete_map={'Year': 'steelblue',
                                            'Registered Vehicles': 'crimson',
                                            'Driver Fault Ratio': 'darkorange',
                                            'Vehicles Involved': 'seagreen'})
...
with tab7:
    ...
    fig_who.update_layout(height=600, margin=dict(l=0, r=0, t=40, b=0))
